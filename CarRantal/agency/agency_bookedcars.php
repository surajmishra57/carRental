...
<body style="background-color: white;">
    ...
    <div class="container">
        <div class="row">
            <div class="col sm-12">
                <h3 class="alert-warning p-2 mt-2 sticky-top text-center"><i class="fas fa-store"></i> Booked Cars
                    Information</h3>
                <?php
                    if(isset($_SESSION['Acancel']))
                    {
                        echo $_SESSION['Acancel'];
                        unset($_SESSION['Acancel']);
                    }
                 ?>
                <?php
                        include_once("../dbconnection.php");
                        
                        $sql = "SELECT booked_vehicle_tb.id,vehicle_tb.v_image,vehicle_tb.v_model,vehicle_tb.v_number,vehicle_tb.v_capacity,booked_vehicle_tb.rent,booked_vehicle_tb.s_date,booked_vehicle_tb.end_date, booked_vehicle_tb.user_email FROM vehicle_tb INNER JOIN booked_vehicle_tb ON booked_vehicle_tb.car_id= vehicle_tb.v_id WHERE booked_vehicle_tb.agency='".$_SESSION['Agency']."'";
                        ...
                        if ($result!==false && $result->num_rows > 0) {
                            ...
                            echo '<th scope="col">Booked By</th>';
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td><a href='./agency_bookingCancel.php?id=$i' class='btn btn-danger btn-sm btn-del'>Cancel</a></td>";
